...
function OrderHotel() {
   ...
   const increment = (page) => {
    ...
    const getBypage = async (page)=>{
      const response= await orderService.getOrderHotelByPage(page) 
  ...
  const handleUpdate = async (id)=>{
    await orderService.updateHotelService(id) 
    window.location.reload()
  }
  const handleDelete = async (id)=>{
    const answer = window.confirm("bạn thật sự muốn xóa");
    if (answer) {
      await orderService.removeHotelService(id) 
      window.location.reload()
  } 
